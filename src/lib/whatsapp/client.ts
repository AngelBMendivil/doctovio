...
import type { WhatsappCredentials } from "./routing";

const { apiVersion: API_VERSION, phoneNumberId: PHONE_NUMBER_ID, accessToken: ACCESS_TOKEN } = WHATSAPP_CONFIG;

/**
 * Credenciales del entorno. Son el respaldo para la instalación de un solo
 * consultorio; con varios, cada uno manda con las suyas y se pasan por
 * parámetro (ver `credentialsForOrganization` en routing.ts).
 *
 * Importa que cada consultorio envíe desde SU número: si todos salieran del
 * mismo, el paciente de un consultorio recibiría mensajes desde el número de
 * otro.
 */
function envCredentials(): WhatsappCredentials | null {
  if (!PHONE_NUMBER_ID || !ACCESS_TOKEN) return null;
  return { phoneNumberId: PHONE_NUMBER_ID, accessToken: ACCESS_TOKEN };
}
...
async function post(body: unknown, creds?: WhatsappCredentials | null): Promise<{ externalId: string }> {
  const use = creds ?? envCredentials();
  if (!use) {
    throw new WhatsAppError("WhatsApp no está configurado en este entorno.", 0, false);
  }

  const res = await fetch(`https://graph.facebook.com/${API_VERSION}/${use.phoneNumberId}/messages`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${use.accessToken}`,
 ...
 *
 * `creds` opcional: sin él se usa el número del entorno (instalación de un
 * solo consultorio). Con varios consultorios, quien llama pasa el del suyo.
 */
export async function sendText(to: string, body: string, creds?: WhatsappCredentials | null) {
  return post(
    {
      to: toWaId(to),
      type: "text",
...
export async function sendOptions(
  ...
  if (options.length === 0) return sendText(to, body, creds);
